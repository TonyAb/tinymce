...
describe('browser.tinymce.core.SelectionOverridesTest', () => {
  ...
  const getEditorTop = (editor: Editor) => Scroll.get(TinyDom.document(editor)).top;
  ...
  it('TINY-7062: place cursor in ce=true element, click to left of ce=false parent element', () => {
    ...
    const editorTop = getEditorTop(editor);

    // Check fake caret has been added before
    assertSelectionIsCaretBlock(editor, 'before');
    // Check scroll position has not changed
    assert.equal(getEditorTop(editor), editorTop);
